...
//
// Each persona is a smooth Gaussian "bump" (peak height = normalized ROAS) on a
// shared grid, combined via max() so overlapping bumps don't add up unrealistically
// and each mountain's true peak height still reflects its own ROAS exactly.
// Elevation is colored on a red -> orange -> yellow -> green scale (low ROAS/valley
// = red, high ROAS/peak = green), so the mountains visually vary with ROAS, not an
// arbitrary per-persona color. When a single persona is selected via the pills,
// non-selected mountains are recolored to flat grey via a separate `surfacecolor`
// array (decoupled from the `z` elevation, which still shows every persona's shape).
function renderPersonaChart(dateFilteredRows) {
  ...
  for (const yy of coords) {
    ...
    for (const xx of coords) {
      ...
      for (const pt of points) {
        const d2 = ((xx - pt.x) ** 2 + (yy - pt.y) ** 2) / (2 * sigma * sigma);
  ...
  // flat grey; values [0,1] (elevation of active/visible mountains) render the
  // red (valley/low ROAS) -> orange -> yellow -> green (peak/high ROAS) gradient.
  const colorscale = [
    [0, GREY_OUT], [0.4999, GREY_OUT],
    [0.5, "#FF4444"], [0.67, "#FFA733"], [0.84, "#FFD54D"], [1, "#4CAF50"],
  ];
